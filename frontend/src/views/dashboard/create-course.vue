...
import { useCreateCourse } from "@/composables/use-courses";
...
const handleSubmit = (e: Event) => {
	...
	mutate(
		...
		{
			onSuccess: () => {
				// Success is handled by the mutation - it will redirect
			},
			onError: (err: any) => {
				if (err.response?.data?.errors) {
					Object.assign(errors, err.response.data.errors);
				} else {
					errors.general = err.response?.data?.message || 'Failed to create course. Please try again.';
				}
			},
		}
	);
...
<template>
	<div class="max-w-4xl mx-auto">
		...
		<div class="mb-8">
			...
			<p class="mt-2 text-gray-600">Fill in the details below to create a new course for your students.</p>
		</div>
		...
		<div class="mt-6 bg-blue-50 rounded-xl p-6 border border-blue-100">
			...
			<ul class="space-y-2 text-sm text-blue-800">
				...
				<li class="flex items-start gap-2">
					...
					Write a compelling description that highlights the key benefits
				</li>
				<li class="flex items-start gap-2">
					<svg class="w-4 h-4 mt-0.5 text-blue-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
						<path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
					</svg>
					Start with Draft status to perfect your content before publishing
				</li>
			</ul>
		</div>
	</div>
